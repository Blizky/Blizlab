...
export function createLayersTool(opts) {
  ...
  function ensureBackgroundEditor() {
      ...
      <div class="layer-bg-modal" role="dialog" aria-modal="true" aria-label="Layer brush editor">
        <div class="layer-bg-head">
          <div class="layer-bg-controls-row">
            <div class="pill-toggle layer-bg-brush-toggle" role="tablist" aria-label="Brush type">
              <button type="button" data-bg-brush-mode="erase" class="active"><img src="./svg/eraser_line.svg" alt="">Erase</button>
              <button type="button" data-bg-brush-mode="restore"><img src="./svg/paint_brush_line.svg" alt="">Restore</button>
            </div>
        ...
        <div class="layer-bg-foot">
          <div class="layer-bg-meta layer-bg-view-left">
            ...
            <div class="layer-bg-hint"><img src="./svg/hand_line.svg" alt="">Hold Space to move</div>
          </div>
          <div class="layer-bg-actions layer-bg-actions-center">
            <button type="button" class="secondary-btn" data-bg-cutout><img src="./svg/eraser_ai_line.svg" alt="">Cutout</button>
            <button type="button" class="secondary-btn" data-bg-chroma><img src="./svg/color_picker_line.svg" alt="">Color</button>
            <button type="button" class="secondary-btn" data-bg-reset><img src="./svg/recycle_line.svg" alt="">Reset</button>
          </div>
          <div class="layer-bg-actions layer-bg-actions-right">
            ...
            <button type="button" class="layer-bg-info-icon" data-bg-info title="About apply behavior" aria-label="About apply behavior"><img src="./svg/information_line.svg" alt=""></button>
          </div>
        </div>
      </div>
    `;
    ...
    window.addEventListener("keydown", (evt) => {
      ...
      if (isTextEntryTarget(evt.target)) return;
    ...
    window.addEventListener("blur", () => {
      if (!bgEditor.spaceMove && !bgEditor.panning) return;
      bgEditor.spaceMove = false;
      bgEditor.panning = false;
      bgEditor.pointerId = null;
      editor.updateModeButtons();
  ...
  function updateCanvasSize(ratio) {
    ...
    const viewportHeightBudget = Math.max(280, window.innerHeight - 190);
    const viewportLimitedWidth = Math.floor(viewportHeightBudget * ratioValue);
    const nextMaxWidth = Math.max(220, Math.min(designMaxWidth, viewportLimitedWidth));
    canvasWrap.style.width = `min(100%, ${nextMaxWidth}px)`;
    canvasWrap.style.maxWidth = `${nextMaxWidth}px`;
  ...
  function drawLayers(targetCtx, progress = 0, parallaxEnabled = false, motionType = "zoom", intensity = 1, lockTopLayer = false) {
